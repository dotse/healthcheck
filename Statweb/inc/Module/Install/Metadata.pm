...
sub feature {
    ...
    my $count = 0;
    push @$features,
      ($name => [map { ref($_) ? (ref($_) eq 'HASH') ? %$_ : @$_ : $_ } @$mods]
      );
...
sub license_from {
      ...
      )
    {
        my $license_text = $1;
        my @phrases      = (
'under the same (?:terms|license) as (?:perl|the perl programming language) itself'
              => 'perl',
            1,
...
sub _perl_version {
    ...
    $v =~
s/^([1-9])\.([1-9]\d?\d?)\.(0|[1-9]\d?\d?)$/sprintf("%d.%03d%03d",$1,$2,$3 || 0)/e;
    $v =~ s/(\.\d\d\d)000$/$1/;
    $v =~ s/_.+$//;
    if (ref($v)) {
...
sub write_mymeta_json {
    ...
    Module::Install::_write('MYMETA.json',
